...
def get_stats_helper(
    ...
    '''
    @return[pg_strategy][version][model]
    '''
    data = []
        ...
        for iv, v in enumerate(npu_versions):
            ...
            for im, model in enumerate(models):
                stats, slo_scale = get_optimal_energy_stat(
                    model, v, workload, pg, batch_size, prefill_or_decode
                )
...
def plot_data(
    ...
    if plot_legend:
        ...
        lg=ax.legend(
            ...
            bbox_to_anchor=(2.6, 1.48), frameon=True,
        )
        frame = lg.get_frame()
...
def main(argv: list[str]):
    ...
    pdf_filename = f"outputs/eval_sens_npu_gen.pdf"
