...
def get_energy_eff_llm_training() -> tuple[list[list[float]], ...]:
    ...
    for im, model in enumerate(LLM_MODELS):
        for iv, v in enumerate(NPU_VERSIONS):
            ...
            value = stats["energy_stats"][__PG_STRATEGY.value]["component_stats"]["hbm_temp_util"]
...
def plot_data(
    ...
    bar_label_offset_xy: Sequence[Sequence[float]] = ((0, 0), (0, 0), (0, 0), (0, 0)),
):
    ...
    # data = np.array(data)
    # print(data)
    # print("min:", np.min(1-data))
    # print("max:", np.max(1-data))
    # print("avg:", gmean(1-data, axis=None))  # type: ignore
    ...
    ax.bar_label(ax.containers[0], labels=[f"{round(x*100):.0f}" for x in data[0]], label_type="edge", fontsize=fontsize_barlabel, position=bar_label_offset_xy[0])  # type: ignore

    YValues2 = data[1]
    ax.bar( XValues-BarWidth/2, YValues2, BarWidth, edgecolor=edgecolor, linewidth=linewidth, color=color2, hatch="--")
    ax.bar_label(ax.containers[1], labels=[f"{round(x*100):.0f}" for x in data[1]], label_type="edge", fontsize=fontsize_barlabel, position=bar_label_offset_xy[1])  # type: ignore

    YValues3 = data[2]
    ax.bar( XValues+BarWidth/2, YValues3, BarWidth, edgecolor=edgecolor, linewidth=linewidth, color=color3, hatch="///")
    ax.bar_label(ax.containers[2], labels=[f"{round(x*100):.0f}" for x in data[2]], label_type="edge", fontsize=fontsize_barlabel, position=bar_label_offset_xy[2])  # type: ignore

    YValues4 = data[3]
    ax.bar( XValues+BarWidth/2*3, YValues4, BarWidth, edgecolor=edgecolor, linewidth=linewidth, color=color4, hatch="\\\\")
    ax.bar_label(ax.containers[3], labels=[f"{round(x*100):.0f}" for x in data[3]], label_type="edge", fontsize=fontsize_barlabel, position=bar_label_offset_xy[3])  # type: ignore
...
def main(argv: list[str]):
    ...
    bar_label_offset_xy = [[ [0.0, 0.0] ] * len(all_energy[0])] * len(all_energy)
    bar_label_offset_xy[1] = [
        [-3, 0],  # offset for bar label of data[0]
        [-1, 0],  # offset for bar label of data[1]
        [1, 0],  # offset for bar label of data[2]
        [3, 0],  # offset for bar label of data[3]
    ]
    ...
    pdf_filename = f"outputs/hbm_temporal_utilization.pdf"
    PDF = PdfPages( pdf_filename )

    graphs = Figure.subplots( nrows=NROWS, ncols=NCOLS )

    for idx, (xnames, energy_data, slo_scale_data, ylabel, title, ylim, logy, barlabel_offset) in enumerate(zip(
        all_XNames, all_energy, all_slo_scale, all_YLabels, all_Titles, all_ylims, all_logy, bar_label_offset_xy
    )):
        # Graph = Figure.add_subplot(NROWS, NCOLS, idx + 1)
        Graph = graphs[idx]
        plot_data(Graph, energy_data, slo_scale_data, xnames, ylabel, title, ylim, logy, barlabel_offset)
